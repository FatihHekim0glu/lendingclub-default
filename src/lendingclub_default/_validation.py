...
from lendingclub_default._exceptions import InsufficientDataError, ValidationError
...
def ensure_series(
    ...
    *,
    ...
    if isinstance(data, pd.Series):
        ...
        # Documented pandas boundary: ``data`` is a sequence/mapping coercible to
        # a Series; pandas-stubs cannot narrow ``object``, so we cast at the edge.
        series = pd.Series(cast("Any", data))
...
def ensure_dataframe(
    ...
    *,
    ...
    if isinstance(data, pd.DataFrame):
        ...
        # Documented pandas boundary: ``data`` is a mapping coercible to a
        # DataFrame; pandas-stubs cannot narrow ``object``, so we cast at the edge.
...
def validate_min_obs(data: pd.DataFrame, min_obs: int, *, name: str = "data") -> None:
    ...
    Used to guard fitting and evaluation: a temporal train/test fold must be
    non-empty and large enough to estimate stable transform statistics (e.g. an
    out-of-fold target encoding), so callers pass a positive ``min_obs``.
